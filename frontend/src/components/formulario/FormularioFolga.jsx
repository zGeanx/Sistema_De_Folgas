...
import { Moon, Sun, Sunset, User, Send, Check } from 'lucide-react';
  ...
  return (
    <div className="page-enter space-y-6">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Dia da Semana — horizontal scroll chips */}
        <div className="space-y-2">
          <label className="text-sm font-semibold text-moonlight">
            Dia da semana
          </label>
            ...
            {Object.entries(DIAS_SEMANA).map(([key, label]) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-3 rounded-xl text-xs font-semibold border transition-all duration-200 flex flex-col items-center gap-0.5 ${
                    isSelected
                      ? 'chip-selected'
                      : 'chip-idle hover:border-slate-600 hover:text-slate-300'
                  }`}
                >
                  <span className="font-bold text-[13px]">{DIA_SHORT[key]}</span>
                  <span className={`text-[9px] ${isSelected ? 'text-amber-300/80' : 'text-silver-mist/60'}`}>
                    {isWeekend ? 'Fim de sem.' : 'Útil'}
                  </span>
                </button>
              );
            })}
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="glass border-amber-gold/20 text-moonlight max-w-sm mx-4 rounded-2xl">
          <DialogHeader>
            ...
            <DialogTitle className="text-center text-lg font-bold text-moonlight">
